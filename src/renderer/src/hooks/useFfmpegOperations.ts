...
async function writeChaptersFfmetadata(
  outDir: string,
...
export async function maybeMkDeepOutDir({
  outputDir,
  fileOutPath,
}: {
  outputDir: string;
...
function useFfmpegOperations({
  ...
  ffmpegHwaccel,
  _activeVideoStreamIndex,
  _activeAudioStreamIndexes,
}: {
  ...
  _activeVideoStreamIndex: number | undefined;
  _activeAudioStreamIndexes: Set<number>;
}) {
  // Silence unused parameter warnings (used in cutMultiple via closure)
  void _activeVideoStreamIndex;
  void _activeAudioStreamIndexes;
  const shouldSkipExistingFile = useCallback(
    async (path: string) => {
  ...
  const concatFiles = useCallback(
    async ({
      ...
      const durations = await pMap(
        paths,
        async (path) => (await getDuration(path)) ?? 0,
        { concurrency: 1 },
      );
      ...
      if (chapters) {
        const chaptersWithNames = chapters.map((chapter, i) => ({
          ...chapter,
      ...
      try {
        ...
        const { streamIdsToCopy, excludedStreamIds } = getStreamIdsToCopy({
          streams,
          includeAllStreams,
        });
        const mapStreamsArgs = getMapStreamsArgs({
          ...
          copyFileStreams: [
            { path: metadataFromPath, streamIds: streamIdsToCopy },
          ],
          outFormat,
        ...
        const result = await runFfmpegConcat({
          ffmpegArgs,
          concatTxt,
          totalDuration,
          onProgress,
  ...
  const losslessCutSingle = useCallback(
    async ({
      keyframeCut: ssBeforeInput,
      avoidNegativeTs,
      copyFileStreams,
      cutFrom,
      cutTo,
      ...
      const effectiveAvoidNegativeTs = getEffectiveAvoidNegativeTs({
        avoidNegativeTs,
        allFilesMeta,
      ...
      function mapInputStreamIndexToOutputIndex(
        inputFilePath: string,
        inputFileStreamIndex: number,
      ) {
        let streamCount = 0;
        // Count copied streams of all files until this input file
        const foundFile = copyFileStreamsFiltered.find(
          ({ path: path2, streamIds }) => {
  ...
  const cutEncodeSmartPart = useCallback(
    async ({
      cutFrom,
      cutTo,
      outPath,
  ...
  // Lossy cut function that uses FFmpeg filters for re-encoding with video/audio filters
  const lossyCutSingle = useCallback(
    async ({
      cutFrom,
      cutTo,
      outPath,
      ...
      allFilesMeta,
      transferTimestamps: transferTimestampsFn,
    }: {
      cutFrom: number;
      cutTo: number;
      outPath: string;
      ...
      const audioStreamIndexes = new Set<number>(
        audioStreams.map((s: { index: number }) => s.index),
      );
      const subtitleStreamIndexes = new Set<number>(
        subtitleStreams.map((s: { index: number }) => s.index),
      );

      // Build copyFileStreams for the main file
      const copyFileStreams: CopyfileStreams = [
        {
          path: filePath,
          streamIds: [
            ...(videoStreamIndex != null ? [videoStreamIndex] : []),
            ...audioStreamIndexes,
      ...
      logStdoutStderr({ stdout: new Uint8Array(), stderr: new Uint8Array() }); // placeholder

      const transferDuration = isDurationValid(fileDuration)
        ? fileDuration
        : undefined;
      await transferTimestampsFn({
        inPath: filePath,
        outPath,
        cutFrom,
        cutTo,
  ...
  const cutMultiple = useCallback(
    async ({
      ...
      function onSingleProgress(id: number, singleProgress: number) {
        ...
        return onTotalProgress(
          sum(Object.values(singleProgresses)) / segments.length,
        );
      }
      ...
      const cutSegment = async (
        ...
        const { streams } = allFilesMeta[filePath]!;
        const streamsToCopyFromMainFile = copyFileStreams
        ...
        const sourceCodecParams = await getCodecParams({
          path: filePath,
          fileDuration,
          streams: streamsToCopyFromMainFile,
        });
        const { videoStream, videoTimebase } = sourceCodecParams;

        const videoCodec = lossyMode
          ? lossyMode.videoEncoder
          : sourceCodecParams.videoCodec;
        ...
        async function cutEncodeSmartPartWrapper({
          cutFrom,
          cutTo: cutToWrapper,
          outPath,
        }: {
          ...
          await cutEncodeSmartPart({
            ...
            videoCodec,
            videoBitrate:
              encCustomBitrate != null
                ? encCustomBitrate * 1000
                : sourceCodecParams.videoBitrate,
            videoStreamIndex: videoStream.index,
            videoTimebase: sourceCodecParams.videoTimebase,
            allFilesMeta,
            copyFileStreams: copyFileStreamsFiltered,
            ffmpegExperimental,
        ...
        const cutEncodeWholePart = async () => {
          await cutEncodeSmartPartWrapper({
            cutFrom: desiredCutFrom,
            cutTo,
            outPath: finalOutPath,
          });
        ...
        const { losslessCutFrom, segmentNeedsSmartCut } = await needsSmartCut({
          path: filePath,
          desiredCutFrom,
          videoStream,
        });
        ...
        const smartCutEncodedPartOutPath = getSuffixedOutPath({
          customOutDir,
          filePath,
          nameSuffix: `smartcut-segment-encode-${i}${ext}`,
        });
        const smartCutSegmentsToConcat = [
        ...
        try {
          ...
          const encodeCutToSafe = Math.max(
            desiredCutFrom + frameDuration,
          ...
          await cutEncodeSmartPartWrapper({
            cutFrom: desiredCutFrom,
            cutTo: encodeCutToSafe,
            outPath: smartCutEncodedPartOutPath,
          });
          ...
          await concatFiles({
            ...
            outDir: outputDir,
            outPath: finalOutPath,
            metadataFromPath: losslessPartOutPath,
            outFormat,
            includeAllStreams: true,
            streams: streamsAfterCut,
            ffmpegExperimental,
            preserveMovData,
            movFastStart,
            chapters,
  ...
  const concatCutSegments = useCallback(
    async ({
      customOutDir,
      outFormat,
      segmentPaths,
      ...
      const chapters = await createChaptersFromSegments({
        paths: segmentPaths,
        defaultChapterNames: chapterNames,
      });
  ...
  const html5ifyDummy = useCallback(
    async ({
    ...
    async ({
      ...
      if (speed === "fastest") {
        const path = getSuffixedOutPath({
          customOutDir,
          filePath: filePathArg,
          nameSuffix: `${html5ifiedPrefix}${html5dummySuffix}.mkv`,
        });
        await html5ifyDummy({
          filePath: filePathArg,
          outPath: path,
      ...
      const duration = await getDuration(filePathArg);
      appendFfmpegCommandLog(ffmpegArgs);
      const { stdout } = await runFfmpegWithProgress({
        ffmpegArgs,
        duration,
        onProgress,
      });
  ...
  const fixInvalidDuration = useCallback(
    async ({
      filePath: filePathArg,
      outPath,
      onProgress,
  ...
  const extractNonAttachmentStreams = useCallback(
    async ({
      customOutDir,
      streams,
    }: {
  ...
  const extractAttachmentStreams = useCallback(
    async ({
      customOutDir,
      streams,
    }: {
  ...
  const extractStreams = useCallback(
    async ({
      customOutDir,
      streams,
    }: {
