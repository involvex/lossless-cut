...
export default function useUserSettingsRoot() {
  ...
  function safeSetConfig<T extends keyof Config>(
    ...
    try {
      ...
      errorToast(
        i18n.t(
          "Unable to save your preferences. Try to disable any anti-virus",
        ),
      );
    }
  }
  ...
  const safeGetConfigInitial =
    <T extends keyof Config>(key: T) =>
    () =>
      safeGetConfig(key);

  const [lastAppVersion, setLastAppVersion] = useState(
    safeGetConfigInitial("lastAppVersion"),
  );
  useEffect(() => safeSetConfig({ lastAppVersion }), [lastAppVersion]);
  const [captureFormat, setCaptureFormat] = useState(
    safeGetConfigInitial("captureFormat"),
  );
  useEffect(() => safeSetConfig({ captureFormat }), [captureFormat]);
  const [recentCustomOutDirs, setRecentCustomOutDirs] = useState(
    safeGetConfigInitial("recentCustomOutDirs"),
  );
  useEffect(
    () => safeSetConfig({ recentCustomOutDirs }),
    [recentCustomOutDirs],
  );
  const [enableCustomOutDir, setEnableCustomOutDir] = useState(
    safeGetConfigInitial("enableCustomOutDir"),
  );
  useEffect(() => safeSetConfig({ enableCustomOutDir }), [enableCustomOutDir]);
  const [keyframeCut, setKeyframeCut] = useState(
    safeGetConfigInitial("keyframeCut"),
  );
  useEffect(() => safeSetConfig({ keyframeCut }), [keyframeCut]);
  const [preserveMetadata, setPreserveMetadata] = useState(
    safeGetConfigInitial("preserveMetadata"),
  );
  useEffect(() => safeSetConfig({ preserveMetadata }), [preserveMetadata]);
  const [preserveMetadataOnMerge, setPreserveMetadataOnMerge] = useState(
    safeGetConfigInitial("preserveMetadataOnMerge"),
  );
  useEffect(
    () => safeSetConfig({ preserveMetadataOnMerge }),
    [preserveMetadataOnMerge],
  );
  const [preserveMovData, setPreserveMovData] = useState(
    safeGetConfigInitial("preserveMovData"),
  );
  useEffect(() => safeSetConfig({ preserveMovData }), [preserveMovData]);
  const [preserveChapters, setPreserveChapters] = useState(
    safeGetConfigInitial("preserveChapters"),
  );
  useEffect(() => safeSetConfig({ preserveChapters }), [preserveChapters]);
  const [movFastStart, setMovFastStart] = useState(
    safeGetConfigInitial("movFastStart"),
  );
  useEffect(() => safeSetConfig({ movFastStart }), [movFastStart]);
  const [avoidNegativeTs, setAvoidNegativeTs] = useState(
    safeGetConfigInitial("avoidNegativeTs"),
  );
  ...
  const [timecodeFormat, setTimecodeFormat] = useState(
    safeGetConfigInitial("timecodeFormat"),
  );
  useEffect(() => safeSetConfig({ timecodeFormat }), [timecodeFormat]);
  const [invertCutSegments, setInvertCutSegments] = useState(
    safeGetConfigInitial("invertCutSegments"),
  );
  useEffect(() => safeSetConfig({ invertCutSegments }), [invertCutSegments]);
  const [autoExportExtraStreams, setAutoExportExtraStreams] = useState(
    safeGetConfigInitial("autoExportExtraStreams"),
  );
  useEffect(
    () => safeSetConfig({ autoExportExtraStreams }),
    [autoExportExtraStreams],
  );
  const [askBeforeClose, setAskBeforeClose] = useState(
    safeGetConfigInitial("askBeforeClose"),
  );
  useEffect(() => safeSetConfig({ askBeforeClose }), [askBeforeClose]);
  const [enableImportChapters, setEnableImportChapters] = useState(
    safeGetConfigInitial("enableImportChapters"),
  );
  useEffect(
    () => safeSetConfig({ enableImportChapters }),
    [enableImportChapters],
  );
  const [enableAskForFileOpenAction, setEnableAskForFileOpenAction] = useState(
    safeGetConfigInitial("enableAskForFileOpenAction"),
  );
  useEffect(
  ...
  const [playbackVolume, setPlaybackVolume] = useState(
    safeGetConfigInitial("playbackVolume"),
  );
  useEffect(() => safeSetConfig({ playbackVolume }), [playbackVolume]);
  const [autoSaveProjectFile, setAutoSaveProjectFile] = useState(
    safeGetConfigInitial("autoSaveProjectFile"),
  );
  useEffect(
    () => safeSetConfig({ autoSaveProjectFile }),
    [autoSaveProjectFile],
  );
  const [wheelSensitivity, setWheelSensitivity] = useState(
    safeGetConfigInitial("wheelSensitivity"),
  );
  useEffect(() => safeSetConfig({ wheelSensitivity }), [wheelSensitivity]);
  const [waveformHeight, setWaveformHeight] = useState(
    safeGetConfigInitial("waveformHeight"),
  );
  useEffect(() => safeSetConfig({ waveformHeight }), [waveformHeight]);
  const [invertTimelineScroll, setInvertTimelineScroll] = useState(
    safeGetConfigInitial("invertTimelineScroll"),
  );
  useEffect(
  ...
  const [ffmpegExperimental, setFfmpegExperimental] = useState(
    safeGetConfigInitial("ffmpegExperimental"),
  );
  useEffect(() => safeSetConfig({ ffmpegExperimental }), [ffmpegExperimental]);
  const [hideNotifications, setHideNotifications] = useState(
    safeGetConfigInitial("hideNotifications"),
  );
  useEffect(() => safeSetConfig({ hideNotifications }), [hideNotifications]);
  const [hideOsNotifications, setHideOsNotifications] = useState(
    safeGetConfigInitial("hideOsNotifications"),
  );
  useEffect(
    () => safeSetConfig({ hideOsNotifications }),
    [hideOsNotifications],
  );
  const [autoLoadTimecode, setAutoLoadTimecode] = useState(
    safeGetConfigInitial("autoLoadTimecode"),
  );
  useEffect(() => safeSetConfig({ autoLoadTimecode }), [autoLoadTimecode]);
  const [autoDeleteMergedSegments, setAutoDeleteMergedSegments] = useState(
    safeGetConfigInitial("autoDeleteMergedSegments"),
  );
  useEffect(
    () => safeSetConfig({ autoDeleteMergedSegments }),
    [autoDeleteMergedSegments],
  );
  const [exportConfirmEnabled, setExportConfirmEnabled] = useState(
    safeGetConfigInitial("exportConfirmEnabled"),
  );
  useEffect(
    () => safeSetConfig({ exportConfirmEnabled }),
    [exportConfirmEnabled],
  );
  const [segmentsToChapters, setSegmentsToChapters] = useState(
    safeGetConfigInitial("segmentsToChapters"),
  );
  useEffect(() => safeSetConfig({ segmentsToChapters }), [segmentsToChapters]);
  const [simpleMode, setSimpleMode] = useState(
    safeGetConfigInitial("simpleMode"),
  );
  useEffect(() => safeSetConfig({ simpleMode }), [simpleMode]);
  const [cutFileTemplate, setCutFileTemplate] = useState(
    safeGetConfigInitial("outSegTemplate"),
  );
  useEffect(
    () => safeSetConfig({ outSegTemplate: cutFileTemplate }),
    [cutFileTemplate],
  );
  const [cutMergedFileTemplate, setCutMergedFileTemplate] = useState(
    safeGetConfigInitial("mergedFileTemplate"),
  );
  useEffect(
    () => safeSetConfig({ mergedFileTemplate: cutMergedFileTemplate }),
    [cutMergedFileTemplate],
  );
  const [mergedFileTemplate, setMergedFileTemplate] = useState(
    safeGetConfigInitial("mergedFilesTemplate"),
  );
  useEffect(
    () => safeSetConfig({ mergedFilesTemplate: mergedFileTemplate }),
    [mergedFileTemplate],
  );
  const [keyboardSeekAccFactor, setKeyboardSeekAccFactor] = useState(
    safeGetConfigInitial("keyboardSeekAccFactor"),
  );
  useEffect(
    () => safeSetConfig({ keyboardSeekAccFactor }),
    [keyboardSeekAccFactor],
  );
  const [keyboardNormalSeekSpeed, setKeyboardNormalSeekSpeed] = useState(
    safeGetConfigInitial("keyboardNormalSeekSpeed"),
  );
  useEffect(
    () => safeSetConfig({ keyboardNormalSeekSpeed }),
    [keyboardNormalSeekSpeed],
  );
  const [keyboardSeekSpeed2, setKeyboardSeekSpeed2] = useState(
    safeGetConfigInitial("keyboardSeekSpeed2"),
  );
  useEffect(() => safeSetConfig({ keyboardSeekSpeed2 }), [keyboardSeekSpeed2]);
  const [keyboardSeekSpeed3, setKeyboardSeekSpeed3] = useState(
    safeGetConfigInitial("keyboardSeekSpeed3"),
  );
  ...
  ] = useState(safeGetConfigInitial("treatInputFileModifiedTimeAsStart"));
  useEffect(
  ...
  ] = useState(safeGetConfigInitial("treatOutputFileModifiedTimeAsStart"));
  useEffect(
    () => safeSetConfig({ treatOutputFileModifiedTimeAsStart }),
    [treatOutputFileModifiedTimeAsStart],
  );

  const [outFormatLocked, setOutFormatLocked] = useState(
    safeGetConfigInitial("outFormatLocked"),
  );
  useEffect(() => safeSetConfig({ outFormatLocked }), [outFormatLocked]);
  const [safeOutputFileName, setSafeOutputFileName] = useState(
    safeGetConfigInitial("safeOutputFileName"),
  );
  useEffect(() => safeSetConfig({ safeOutputFileName }), [safeOutputFileName]);
  const [enableAutoHtml5ify, setEnableAutoHtml5ify] = useState(
    safeGetConfigInitial("enableAutoHtml5ify"),
  );
  useEffect(() => safeSetConfig({ enableAutoHtml5ify }), [enableAutoHtml5ify]);
  const [segmentsToChaptersOnly, setSegmentsToChaptersOnly] = useState(
    safeGetConfigInitial("segmentsToChaptersOnly"),
  );
  useEffect(
  ...
  const [keyBindings, setKeyBindings] = useState(
    safeGetConfigInitial("keyBindings"),
  );
  useEffect(() => safeSetConfig({ keyBindings }), [keyBindings]);
  const [enableSmartCut, setEnableSmartCut] = useState(
    safeGetConfigInitial("enableSmartCut"),
  );
  useEffect(() => safeSetConfig({ enableSmartCut }), [enableSmartCut]);
  const [customFfPath, setCustomFfPath] = useState(
    safeGetConfigInitial("customFfPath"),
  );
  useEffect(() => safeSetConfig({ customFfPath }), [customFfPath]);
  const [storeProjectInWorkingDir, setStoreProjectInWorkingDir] = useState(
    safeGetConfigInitial("storeProjectInWorkingDir"),
  );
  useEffect(
    () => safeSetConfig({ storeProjectInWorkingDir }),
    [storeProjectInWorkingDir],
  );
  const [enableOverwriteOutput, setEnableOverwriteOutput] = useState(
    safeGetConfigInitial("enableOverwriteOutput"),
  );
  useEffect(
    () => safeSetConfig({ enableOverwriteOutput }),
    [enableOverwriteOutput],
  );
  const [mouseWheelZoomModifierKey, setMouseWheelZoomModifierKey] = useState(
    safeGetConfigInitial("mouseWheelZoomModifierKey"),
  );
  useEffect(
    () => safeSetConfig({ mouseWheelZoomModifierKey }),
    [mouseWheelZoomModifierKey],
  );
  const [mouseWheelFrameSeekModifierKey, setMouseWheelFrameSeekModifierKey] =
    useState(safeGetConfigInitial("mouseWheelFrameSeekModifierKey"));
  useEffect(
  ...
  ] = useState(safeGetConfigInitial("mouseWheelKeyframeSeekModifierKey"));
  useEffect(
    () => safeSetConfig({ mouseWheelKeyframeSeekModifierKey }),
    [mouseWheelKeyframeSeekModifierKey],
  );
  const [segmentMouseModifierKey, setSegmentMouseModifierKey] = useState(
    safeGetConfigInitial("segmentMouseModifierKey"),
  );
  useEffect(
    () => safeSetConfig({ segmentMouseModifierKey }),
    [segmentMouseModifierKey],
  );
  const [captureFrameMethod, setCaptureFrameMethod] = useState(
    safeGetConfigInitial("captureFrameMethod"),
  );
  useEffect(() => safeSetConfig({ captureFrameMethod }), [captureFrameMethod]);
  const [captureFrameQuality, setCaptureFrameQuality] = useState(
    safeGetConfigInitial("captureFrameQuality"),
  );
  useEffect(
    () => safeSetConfig({ captureFrameQuality }),
    [captureFrameQuality],
  );
  const [captureFrameFileNameFormat, setCaptureFrameFileNameFormat] = useState(
    safeGetConfigInitial("captureFrameFileNameFormat"),
  );
  useEffect(
    () => safeSetConfig({ captureFrameFileNameFormat }),
    [captureFrameFileNameFormat],
  );
  const [enableNativeHevc, setEnableNativeHevc] = useState(
    safeGetConfigInitial("enableNativeHevc"),
  );
  useEffect(() => safeSetConfig({ enableNativeHevc }), [enableNativeHevc]);
  const [enableUpdateCheck, setEnableUpdateCheck] = useState(
    safeGetConfigInitial("enableUpdateCheck"),
  );
  useEffect(() => safeSetConfig({ enableUpdateCheck }), [enableUpdateCheck]);
  const [cleanupChoices, setCleanupChoices] = useState(
    safeGetConfigInitial("cleanupChoices"),
  );
  useEffect(() => safeSetConfig({ cleanupChoices }), [cleanupChoices]);
  const [allowMultipleInstances, setAllowMultipleInstances] = useState(
    safeGetConfigInitial("allowMultipleInstances"),
  );
  useEffect(
  ...
  const [preferStrongColors, setPreferStrongColors] = useState(
    safeGetConfigInitial("preferStrongColors"),
  );
  useEffect(() => safeSetConfig({ preferStrongColors }), [preferStrongColors]);
  const [outputFileNameMinZeroPadding, setOutputFileNameMinZeroPadding] =
    useState(safeGetConfigInitial("outputFileNameMinZeroPadding"));
  useEffect(
    () => safeSetConfig({ outputFileNameMinZeroPadding }),
    [outputFileNameMinZeroPadding],
  );
  const [cutFromAdjustmentFrames, setCutFromAdjustmentFrames] = useState(
    safeGetConfigInitial("cutFromAdjustmentFrames"),
  );
  useEffect(
    () => safeSetConfig({ cutFromAdjustmentFrames }),
    [cutFromAdjustmentFrames],
  );
  const [cutToAdjustmentFrames, setCutToAdjustmentFrames] = useState(
    safeGetConfigInitial("cutToAdjustmentFrames"),
  );
  useEffect(
    () => safeSetConfig({ cutToAdjustmentFrames }),
    [cutToAdjustmentFrames],
  );
  const [storeWindowBounds, setStoreWindowBounds] = useState(
    safeGetConfigInitial("storeWindowBounds"),
  );
  useEffect(() => safeSetConfig({ storeWindowBounds }), [storeWindowBounds]);
  const [waveformMode, setWaveformMode] = useState(
    safeGetConfigInitial("waveformMode"),
  );
  useEffect(() => safeSetConfig({ waveformMode }), [waveformMode]);
  const [thumbnailsEnabled, setThumbnailsEnabled] = useState(
    safeGetConfigInitial("thumbnailsEnabled"),
  );
  useEffect(() => safeSetConfig({ thumbnailsEnabled }), [thumbnailsEnabled]);
  const [keyframesEnabled, setKeyframesEnabled] = useState(
    safeGetConfigInitial("keyframesEnabled"),
  );
  useEffect(() => safeSetConfig({ keyframesEnabled }), [keyframesEnabled]);
  const [reducedMotion, setReducedMotion] = useState(
    safeGetConfigInitial("reducedMotion"),
  );
  useEffect(() => safeSetConfig({ reducedMotion }), [reducedMotion]);
  const [ffmpegHwaccel, setFfmpegHwaccel] = useState(
    safeGetConfigInitial("ffmpegHwaccel"),
  );
  useEffect(() => safeSetConfig({ ffmpegHwaccel }), [ffmpegHwaccel]);
  const [lossyMode, setLossyMode] = useState(safeGetConfigInitial("lossyMode"));
  useEffect(() => safeSetConfig({ lossyMode }), [lossyMode]);
  ...
  const springAnimation = useMemo<Transition>(
    () => (prefersReducedMotion ? { duration: 0 } : mySpring),
    [prefersReducedMotion],
  );

  const customOutDir = useMemo(
    () => (enableCustomOutDir ? recentCustomOutDirs[0] : undefined),
    [enableCustomOutDir, recentCustomOutDirs],
  );

  const setCustomOutDir = useCallback((newDir: string | undefined) => {
    if (newDir) {
      setRecentCustomOutDirs((prev) =>
        [newDir, ...prev.filter((d) => d !== newDir)].slice(0, 5),
      ); // keep only the 5 most recent dirs
