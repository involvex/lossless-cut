...
const ExportButton = forwardRef<HTMLButtonElement, Props>(
  ({ segmentsToExport, areWeCutting, onClick, style, children }, ref) => {
    ...
    const text =
      autoMerge && segmentsToExport && segmentsToExport.length > 1
        ? t("Export+merge")
        : t("Export");
    ...
    return (
      <button
        ...
        title={title}
      >
        <CutIcon style={{ verticalAlign: "middle", marginRight: ".2em" }} />
        {children ?? text}
      </button>
    );
  },
